  ...
  <div class="container-fluid p-3">
    <form method="Post" action="/php/functions.php">
      <div class="mx-auto col-lg-6 col-sm-12 item-center mt-3 mb-3">
        <h2>Student</h2>
        <?php 
        $id_upate_est = $_POST['id_update'];
        echo'<div class="form-group"> <!-- ID -->
        <label for="id" class="control-label">ID '.$id_upate_est.'</label>
        <input type="hidden" class="form-control" id="id" name="id" value='.$id_upate_est.'  >
        </div>';
        ?>
        ...
        <div class="form-group mt-3"> <!-- Submit Button -->
          <button type="submit" name="submit_update" id="submit_update" class="btn btn-primary">Submit</button>
        </div>
      </div>
    </form>
  </div>
